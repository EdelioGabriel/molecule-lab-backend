...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],          # em produção, restrinja ao origin do frontend
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def build_topology(mol):
    ...
    bonds = []; neighbors = {i: set() for i in range(mol.GetNumAtoms())}
    bond_bt_map = {}
    ...
    for c in range(mol.GetNumAtoms()):
        ...
        for a in range(len(nbrs)):
            for b in range(a+1, len(nbrs)):
                ii, jj = nbrs[a], nbrs[b]
                rci = pos[ii] - pos[c]; rcj = pos[jj] - pos[c]
                r_ci = np.linalg.norm(rci)+1e-12; r_cj = np.linalg.norm(rcj)+1e-12
                cos_t = np.clip(np.dot(rci,rcj)/(r_ci*r_cj), -1.0, 1.0)
                angles.append((ii, c, jj, math.acos(cos_t), k_theta))

    dihedrals = []; seen_dih = set()
    for (p,q), bt in bond_bt_map.items():
        V1, V2, V3 = DIHEDRAL_PARAMS.get(bt, (0.0, 0.0, 1.5))
        if V1 == V2 == V3 == 0.0: continue
        for i in neighbors[p]:
            if i == q: continue
            for l in neighbors[q]:
                if l == p or l == i: continue
                tup = (i,p,q,l); key = min(tup, (l,q,p,i))
                if key in seen_dih: continue
                seen_dih.add(key)
...
def shake_positions(pos, masses, shake_bonds, tol=SHAKE_TOL, max_iter=SHAKE_MAX_ITER):
    for _ in range(max_iter):
        ...
        for i, j, d0 in shake_bonds:
            rij = pos[j] - pos[i]; d2 = float(np.dot(rij, rij)); d0_2 = d0*d0
            if abs(d2 - d0_2) < tol * d0_2: continue
            converged = False
            lam = (d2 - d0_2) / (2.0 * (1/masses[i] + 1/masses[j]) * d2)
            pos[i] += lam / masses[i] * rij; pos[j] -= lam / masses[j] * rij
        if converged: break
    return pos
...
def rattle_velocities(pos, vel, masses, shake_bonds, tol=SHAKE_TOL, max_iter=SHAKE_MAX_ITER):
    for _ in range(max_iter):
        converged = True
        for i, j, d0 in shake_bonds:
            rij = pos[j] - pos[i]; vij = vel[j] - vel[i]; rv = float(np.dot(rij, vij))
            if abs(rv) < tol: continue
            converged = False
            mu = rv / ((1/masses[i] + 1/masses[j]) * d0*d0)
            vel[i] += mu / masses[i] * rij; vel[j] -= mu / masses[j] * rij
        if converged: break
    return vel
...
def baoab_step(pos, vel, masses, forces, dt, gamma, target_T,
    ...
    vel = vel + 0.5*dt*acc
    pos = pos + 0.5*dt*vel
    if shake_bonds: pos = shake_positions(pos, masses, shake_bonds)
    c         = math.exp(-gamma * dt)
    ...
    if shake_bonds: vel = rattle_velocities(pos, vel, masses, shake_bonds)
...
def remove_angular_momentum(pos, vel, masses):
    M = masses.sum(); r_com = (masses[:,None]*pos).sum(0)/M; r_rel = pos - r_com
    ...
    try:
        omega = np.linalg.solve(I, L); vel -= np.cross(omega[None,:], r_rel)
...
class VerletList:
    def __init__(self, r_cut, r_skin):
        self.r_cut = r_cut; self.r_skin = r_skin; self.pos_ref = None
    ...
    def needs_rebuild(self, pos):
        if self.pos_ref is None: return True
    ...
    def build(self, pos, bonded_pairs, one_three, one_four, radii, symbols, charges):
        ...
        pi_l=[]; pj_l=[]; eps_l=[]; sig_l=[]; sc_l=[]; qi_l=[]; qj_l=[]
        n = len(pos)
        for i in range(n):
            for j in range(i+1, n):
                pair = (i,j)
                if pair in bonded_pairs or pair in one_three: continue
                if np.linalg.norm(pos[j]-pos[i]) > r_list: continue
                scale = LJ_SCALE_14 if pair in one_four else 1.0
                sigma, epsilon = lj_params(symbols[i], symbols[j], radii[i], radii[j])
                pi_l.append(i); pj_l.append(j)
                eps_l.append(epsilon); sig_l.append(sigma); sc_l.append(scale)
                qi_l.append(charges[i]); qj_l.append(charges[j])
        self.pair_i=np.array(pi_l,dtype=np.int32); self.pair_j=np.array(pj_l,dtype=np.int32)
        self.epsilons=np.array(eps_l); self.sigmas=np.array(sig_l); self.scales=np.array(sc_l)
        self.charges_i=np.array(qi_l); self.charges_j=np.array(qj_l); self.pos_ref=pos.copy()
...
def _nonbonded_kernel(pos, pair_i, pair_j, epsilons, sigmas, scales,
    ...
    n=pos.shape[0]; forces=np.zeros((n,3)); energy=0.0; ae=np.zeros(n)
    for k in range(len(pair_i)):
        i=pair_i[k]; j=pair_j[k]; sc=scales[k]
        dx=pos[j,0]-pos[i,0]; dy=pos[j,1]-pos[i,1]; dz=pos[j,2]-pos[i,2]
        r2=dx*dx+dy*dy+dz*dz+1e-24; r=math.sqrt(r2)
        if r>r_cut: continue
        sig=sigmas[k]; eps=epsilons[k]
        sr2=(sig/r)**2; sr6=sr2*sr2*sr2; sr12=sr6*sr6
        v_lj=4.0*eps*(sr12-sr6)*sc; f_lj=24.0*eps/r*(2.0*sr12-sr6)*sc
        qi=charges_i[k]; qj=charges_j[k]
        v_c=k_coulomb*qi*qj/r*sc; f_c=k_coulomb*qi*qj/r2*sc
        f_tot=f_lj+f_c; fx=f_tot*dx/r; fy=f_tot*dy/r; fz=f_tot*dz/r
        forces[i,0]-=fx; forces[i,1]-=fy; forces[i,2]-=fz
        forces[j,0]+=fx; forces[j,1]+=fy; forces[j,2]+=fz
        half_e=0.5*(v_lj+v_c); ae[i]+=half_e; ae[j]+=half_e; energy+=v_lj+v_c
    return forces, energy, ae

def nonbonded_forces(pos, vlist):
    if len(vlist.pair_i)==0:
        n=len(pos); return np.zeros((n,3)), 0.0, np.zeros(n)
...
def bonded_forces_and_energy(pos, bonds, angles, dihedrals):
    n=len(pos); forces=np.zeros((n,3)); energy=0.0; ae=np.zeros(n)
    for i,j,r0,De,alpha in bonds:
        rij=pos[j]-pos[i]; r=np.linalg.norm(rij)+1e-12
        u=math.exp(-alpha*(r-r0)); v=De*(1-u)**2; dVdr=2*De*alpha*u*(1-u)
        fij=dVdr*rij/r; forces[i]+=fij; forces[j]-=fij
        energy+=v; ae[i]+=0.5*v; ae[j]+=0.5*v
    for i,c,j,theta0,k_theta in angles:
        rci=pos[i]-pos[c]; rcj=pos[j]-pos[c]
        r_ci=np.linalg.norm(rci)+1e-12; r_cj=np.linalg.norm(rcj)+1e-12
        cos_t=np.clip(np.dot(rci,rcj)/(r_ci*r_cj),-1+1e-9,1-1e-9)
        theta=math.acos(cos_t); sin_t=math.sqrt(max(1-cos_t**2,1e-12))
        dth=theta-theta0; v=0.5*k_theta*dth**2
        coeff=-k_theta*dth/sin_t
        fi=coeff/r_ci*(rcj/r_cj-cos_t*rci/r_ci)
        fj=coeff/r_cj*(rci/r_ci-cos_t*rcj/r_cj)
        forces[i]+=fi; forces[j]+=fj; forces[c]-=(fi+fj)
        energy+=v; ae[i]+=v/3; ae[c]+=v/3; ae[j]+=v/3
    for i,j,k,l,V1,V2,V3 in dihedrals:
        b1=pos[j]-pos[i]; b2=pos[k]-pos[j]; b3=pos[l]-pos[k]
        n1=np.cross(b1,b2); n2=np.cross(b2,b3)
        n1n=np.linalg.norm(n1)+1e-12; n2n=np.linalg.norm(n2)+1e-12
        b2n=np.linalg.norm(b2)+1e-12
        n1h=n1/n1n; b2h=b2/b2n; m1=np.cross(n1h,b2h)
        cos_phi=np.clip(np.dot(n1h,n2/n2n),-1+1e-9,1-1e-9)
        sin_phi=np.dot(m1,n2/n2n); phi=math.atan2(sin_phi,cos_phi)
        v=V1/2*(1+math.cos(phi))+V2/2*(1-math.cos(2*phi))+V3/2*(1+math.cos(3*phi))
        dVdp=-V1/2*math.sin(phi)+V2*math.sin(2*phi)-3*V3/2*math.sin(3*phi)
        energy+=v; ae[i]+=v/4; ae[j]+=v/4; ae[k]+=v/4; ae[l]+=v/4
        if abs(dVdp)<1e-12: continue
        fi=(dVdp*b2n/n1n**2)*n1; fl=-(dVdp*b2n/n2n**2)*n2
        b2l2=b2n**2; fj=(np.dot(b1,b2)/b2l2-1)*fi-np.dot(b3,b2)/b2l2*fl
        fk=-(fi+fj+fl)
        forces[i]+=fi; forces[j]+=fj; forces[k]+=fk; forces[l]+=fl
    return forces, energy, ae
...
def detect_broken_bonds(pos, bonds, break_frac=BREAK_FRAC):
    ...
    for idx,(i,j,r0,De,alpha) in enumerate(bonds):
        r=np.linalg.norm(pos[i]-pos[j]); u=math.exp(-alpha*(r-r0)); V=De*(1-u)**2
...
def run_md(smiles, **kwargs):
    ...
    vlist.build(pos, bonded_pairs, one_three, one_four, radii, symbols, charges)

    vel = initialize_velocities(masses, params["temperature_start"])
    vel = remove_angular_momentum(pos, vel, masses)
    forces, potential, atom_e = forces_and_energy(pos, bonds, angles, dihedrals, vlist)

    rng = np.random.default_rng(42)

    def force_fn(p):
        if vlist.needs_rebuild(p):
            vlist.build(p, bonded_pairs, one_three, one_four, radii, symbols, charges)
        return forces_and_energy(p, bonds, angles, dihedrals, vlist)

    temperatures = []; target_temps = []
...
def analyze(req: AnalyzeRequest):
    ...
    log.info("SMILES gerado: %s", smiles)
